public class Coach {
	...
	/*
	 * Show coach configuration 0 - Number of Rows 1 - Total Seats 2 - Destination 3
	 * - Standard Price 4 - Pensioner Price 5 - Frequent Price 6 - Standard Bookings
	 * 7 - Pensioner Bookings 8 - Frequent Bookings
	 */
	...
	public void coachBook(String[] input) {
		...
		// Process input in loop convert strings to int's and store in processedInputs
		// array
		this.counter = 0;
		...
		if (totalSeatsRequired > this.coachAvaliableSeats()) {
			...
			while (processedInput[0] > this.counter) {
				...
				// Add Seat to ticket, check if array is null, if null do not include null
				// contents
				if (output[0][4] == null) {
	...
	/*
	 * Refund ticket seats expects sting of numbers and whitespace only - will
	 * refund seat/s based of booked ticket type - will error on un-booked
	 * seat/invalid seat
	 */
	public void coachRefund(String input) {
		...
		/*
		 * We had errors bummer - Trim error array - print errors - do nothing further
		 */
		if (error) {
			// Trim array - (removed un-required length)
	...
	/*
	 * Prints Receipt type: true = normal, false = refund destination: the coach
	 * destination 2D input:
	 * 
	 */
	...
	// Ticket Report
	public void coachTicketReport() {
		// Fetch and store required information
		int seatsTotal = (int) this.coach[1];
		int seatsAvaliable = this.coachAvaliableSeats();

		// Simple Calculations
		int seatsBooked = seatsTotal - seatsAvaliable;
		Double seatsPercentageBooked = (Double.valueOf(seatsBooked) / Double.valueOf(seatsTotal)) * 100;
		int seatsPercentageBookedWhole = (int) Math.round(seatsPercentageBooked);

		// Average Book Seat Price Calculations
		Double seatStandardTotal = (int) this.coach[6] * (Double) this.coach[3];
		Double seatPensionerTotal = (int) this.coach[7] * (Double) this.coach[4];
		Double seatFrequentTotal = (int) this.coach[8] * (Double) this.coach[5];

		Double seatAveragePrice = (seatStandardTotal + seatPensionerTotal + seatFrequentTotal) / seatsBooked;
		// Round number to 2 decimal places
		seatAveragePrice = seatAveragePrice * 100;
		seatAveragePrice = (double) Math.round(seatAveragePrice);
		seatAveragePrice = seatAveragePrice / 100;
		...
		System.out.println("Percentage of Seats Booked  : " + seatsPercentageBookedWhole + "%");
		System.out.println("Average Booked Ticket Price : $" + seatAveragePrice);
		System.out.println("");
	}
}